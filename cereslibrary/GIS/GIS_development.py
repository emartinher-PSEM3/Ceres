...
#def GIS_module(longitude,latitude):
...
cmap.set_under('grey')
ax = gplt.choropleth(HUC8_NARS_NLA_FINAL_P, hue='PTL', projection=proj, norm=norm, cmap=cmap, k=5, scheme='quantiles', legend=True, legend_kwargs={'loc': 'lower right'},figsize=(12, 12), vmin=HUC8_NARS_NLA_FINAL_P[HUC8_NARS_NLA_FINAL_P['PTL'] > 0]['PTL'].min(), zorder=-100)#, vmin=0.8, vmax=HUC8_NARS_NLA_FINAL['PTL'].max() , linewidth=0.5, edgecolor='black',)
gplt.pointplot(point, ax=ax, projection=proj, s=300, color='none', edgecolors='r', linewidth='3')
gplt.pointplot(point, ax=ax, projection=proj, color= 'red')
plt.savefig('NARS_NLA_FINAL_P.pdf')
plt.savefig("NARS_NLA_FINAL_P.png", bbox_inches='tight', pad_inches=0.1)

cmap = matplotlib.cm.viridis

cmap.set_under('grey')
ax = gplt.choropleth(HUC8_NARS_NLA_FINAL_N, hue='NH4', projection=proj, norm=norm, cmap=cmap, k=5, scheme='quantiles', legend=True, legend_kwargs={'loc': 'lower right'},figsize=(12, 12), vmin=HUC8_NARS_NLA_FINAL_N[HUC8_NARS_NLA_FINAL_N['NH4'] > 0]['NH4'].min(), zorder=-100)#, vmin=0.8, vmax=HUC8_NARS_NLA_FINAL['PTL'].max() , linewidth=0.5, edgecolor='black',)
gplt.pointplot(point, ax=ax, projection=proj, s=300, color='none', edgecolors='r', linewidth='3')
gplt.pointplot(point, ax=ax, projection=proj, color= 'red')
plt.savefig('NARS_NLA_FINAL_N.pdf')
plt.savefig("NARS_NLA_FINAL_.png", bbox_inches='tight', pad_inches=0.1)
...
NH4_GIS = HUC8_NARS_NLA_FINAL_P_index.loc[HUC8ContPoint, 'NH4']
...
# 
# =============================================================================
#    return  {'HUC8ContPoint':HUC8ContPoint}
...
#          projection='merc',
##           lat_0=, lon_0=
#          llcrnrlon=-126.2, llcrnrlat=24.5, urcrnrlon=-64.5, urcrnrlat=49.5)
#
#m.readshapefile('watershed/huc8sum', 'watershed_data')
##m.drawcoastlines()
#
#
#df_poly = pd.DataFrame({'shapes':[Polygon(np.array(shape), True) for shape in m.watershed_data],
#                    'HUC8': [int(watershed['HUC_8']) for watershed in m.watershed_data_info ]
#                              })
#df_poly_HUC8index = df_poly.set_index('HUC8')
##df_poly_HUC8shapes=df_poly['shapes']
#
#HUC8_total = list(df_poly['HUC8'])
#
#HUC8_dict = dict()
#for i in df_poly_HUC8index.index:
##         print(df_poly_HUC8index[i])
#        HUC8_dict[i] = df_poly_HUC8index.loc[i,'shapes']
#
#HUC8_plot=dict()
#for l in set(HUC8_total): 
##    print(l)       
##    HUC8_path = HUC8_dict[l].get_path()
#    HUC8_path = df_poly_HUC8index.loc[i,'shapes'].get_path()
#    
#    if HUC8_path.contains_point(a):
#        HUC8_plot[a] = df_poly.loc[l,'HUC8']
#        break
#    
